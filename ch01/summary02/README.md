...
신경망 훈련에서 가장 중요한 문제점으로는 **overfitting**(과대적합)이 꼽힌다. 한 model을 train data set을 이용해서 훈련시킨 뒤, 모형이 label을 완벽하게 예측했다고 해도, 미지의 test data에 대해 좋은 예측 성과를 내리라는 보장이 없다.

> train data에 너무 완벽하게 맞춰지면서 training error가 0이 되면, 반대로 실제 데이터에는 제대로 된 예측을 할 수 없을 것이다. 

학습에 사용되지 않은 데이터를 활용하여 평가하는 과정을 **test**한다고 하고, train data 추론이 아닌 경우에도 성능 차이가 없는 능력을 **Generalization**(일반화)라고 한다. 동시에 train data와 test data는 서로 교점이 되는 데이터들이 없기 때문에, test를 Generalization이라고 부르기도 한다.
...
매개변수가 많을수록, 또한 절댓값이 클수록 overfitting이 잘 일어나므로 이를 방지하기 위해 다양한 접근법을 사용할 수 있다.

- 0을 제외한 매개변수 개수를 줄인다.

- 매개변수의 절댓값의 크기를 줄인다.

하지만 매개변수 값 자체에 제한을 거는 방식은 어렵다. 그래서 loss function에 **penalty** 항을 도입하는 좀 더 온건한 접근 방식 **regularization**이 쓰인다. 

$$\lambda || \overline{W} ||^p$$

- $p=2$ 인 경우

    흔히 쓰이는 **L2 regularization**(Tikhonov regularization, 티호노프 정칙화)
    
    {각 매개변수에 정칙화 매개변수 $\lambda >0$ 을 곱한 값}을 제곱한 뒤, loss function에 더해준다.

> 데이터가 단순하고 모델이 복잡하면, training을 하면서 작은 값이었던 weight 값이 점점 증가한다. 

> weight가 커질수록 train data가 모델에 주는 영향력이 커지고, 결국 모델이 train data에 딱 맞춰지게 된다. 이를 local noise(outlier)에 model이 fit되는 현상이라고 표현한다.
